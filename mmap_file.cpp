	...
	namespace largefile
	{
		...
		MMapFile::~MMapFile()
		{
			if (data_)
			{
				if (debug)
				{
					printf("mmap_file destruct,fd:%d,mapped size:%d,data:%p\n", fd_, size_, data_);
				}
		...
		bool MMapFile::map_file(const bool write)
		{
			...
			{
				fprintf(stderr, "enlarge file size failed in map_file,size:%d\n", size_);
				return false;
			};
			...
			if (debug)
				printf("mmap file succeeded ,fd: %d mapped size: %d,data: %p\n", fd_, size_, data_);
			return true;
		}
		...
		bool MMapFile::remap_file()
		{
			...
			{
				if (debug)
					printf("mmap file succeeded ,fd: %d mapped size: %d,data: %p,new data:%p\n", fd_, size_, data_, new_map_data);
			}
